...
def mainfunction_detect():
        speak('detection in process')
        txt = []
        ...
        while True:
            ...
            if ret:
                ...
                opd = make_and_show_inference(img, interpreter, input_details, output_details, category_index)
                arr = opd['detection_classes']
                if (len(arr) > 0) and ((arr[0] != 69)):
                      
                    for i in arr:
                        txt.append(category_index[arr[0]]['name'])
        ...
        if len(txt) > 0:
            pass
            speak("I have found the following details in the video, ")
        else:
            pass
            speak("I have found nothing in the video")
        for i in setArr:
            pass
            speak("there is a " + i)
        print(setArr)
